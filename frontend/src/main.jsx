    ...
    getWsClient().then(c => {
      c.onMessage(data => {
        ...
        if (data.clientId !== clientId) {
          const content = (addr) => addr && `http://${addr}:${import.meta.env.VITE_BACKEND_PORT}/static/downloads?type=${type}&url=${encodeURIComponent(`http://${addr}:${import.meta.env.VITE_BACKEND_PORT}${url}`)}`
          if (type === 'text') {
